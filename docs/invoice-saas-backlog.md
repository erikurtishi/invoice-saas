...
- `0.1.1` (S) Monorepo or two-repo decision; init Vite React TS frontend + Node backend
- `0.1.2` (S) ESLint, Prettier, TypeScript strict mode, Husky pre-commit hooks
- `0.1.3` (S) `.env` handling and config module for both apps; no secrets in repo
- `0.1.4` (S) Install and configure: TanStack Query (with devtools), lucide-react, Motion, GSAP, Tailwind, React Hook Form, Zod
- `0.1.5` (S) Set up TanStack Query `QueryClient` with sane defaults (retry, staleTime, error handling) and global error boundary
...
1. Hostinger VPS vs shared hosting — determines Postgres vs MySQL, and whether Puppeteer runs at all
2. AI generation monthly limit on Premium (suggest 50)
3. Whether tenants can have teammates later — if yes, model `User` as many-per-tenant now even if the UI is single-user
4. Soft delete vs hard delete for clients/products referenced by invoices (recommend soft)
5. Manual grant + Stripe subscription conflict resolution rule (6.3.5)
